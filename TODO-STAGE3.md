...
**RPi 작업 필요 (사용자 직접)**: hospital/clinical 의 `scripts/syncEMR.js`, `syncRounding.js` 등이 RPi 에서 새 mssql 12.5 + uuid 14 로 동작하는지 검증.
```
ssh pi@ewoo-sync
cd /home/pi/ewoo-hospital && git pull && npm install
node scripts/syncEMR.js  # lightweight 한 번 dry-run
# 다음 cron 주기 (10분) 정상 로그 확인
```
