...
export async function parseWeeklyExcel(file, XLSX) {
  const buf = await file.arrayBuffer();
  const wb  = readWorkbook(XLSX, buf);
  const results = [];
...
export async function parseTaxExcel(file, XLSX) {
  const buf = await file.arrayBuffer();
  const wb  = readWorkbook(XLSX, buf);
  const results = [];
