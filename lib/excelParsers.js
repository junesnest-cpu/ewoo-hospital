...
// ─── 위탁진료 환불금 파서 ─────────────────────────────────────────────────────
// 반환: { reportMonth, patients, grandTotal, sheetName }[]
export async function parseRefundExcel(file) {
  ...
  for (const shName of wb.SheetNames) {
    ...
    // 헤더 행에서 컬럼 위치 동적 감지 (병합 셀·형식 변형 대응)
    const hRow = rows[headerRowIdx] || [];
    let colDate = 2, colInst = 3, colTotal = 6, colRefund = 7;
    let colHolder = -1, colBank = -1, colAcct = -1, colPhone = -1, colNote = -1;
    hRow.forEach((cell, ci) => {
      const h = String(cell || "").trim();
      if (/날짜|일자/.test(h) && ci > 0 && ci < 5) colDate = ci;
      if (/기관|병원|의원|의료/.test(h) && ci > 0 && ci < 7) colInst = ci;
      if (/총액/.test(h) && !/환불|환급/.test(h)) colTotal = ci;
      if (/환불|환급/.test(h)) colRefund = ci;
      if (/예금주/.test(h)) colHolder = ci;
      if (/은행/.test(h) && !/보험/.test(h)) colBank = ci;
      if (/계좌/.test(h)) colAcct = ci;
      if (/전화|연락처/.test(h)) colPhone = ci;
      if (/비고/.test(h) && ci > 8) colNote = ci;
    });
    const C = {
      holder:  colHolder >= 0 ? colHolder : (hasDocInfo ? 10 : 8),
      bank:    colBank   >= 0 ? colBank   : (hasDocInfo ? 11 : 9),
      account: colAcct   >= 0 ? colAcct   : (hasDocInfo ? 12 : 10),
      phone:   colPhone  >= 0 ? colPhone  : (hasDocInfo ? 13 : 11),
      note:    colNote   >= 0 ? colNote   : (hasDocInfo ? 14 : 12),
    };

    const patients = [];
    let cur = null;
    let lastDate = "";
    let lastInst = ""; // 진료기관 병합 셀 대응: 마지막 유효값 승계
    ...
    for (let ri = headerRowIdx + 1; ri < rows.length; ri++) {
      ...
      const rawInst  = String(row[colInst] || "").trim();
      const total    = parseNum(row[colTotal]);
      const refund   = parseNum(row[colRefund]);
      ...
      // 병합 셀: 값이 있으면 갱신, 없으면 이전 값 승계
      if (rawInst) lastInst = rawInst;
      const inst = rawInst || lastInst;

      const isSamePatient = cur && String(chartRaw) === cur.chartNo;

      if (chartRaw !== null && chartRaw !== "" && !isSamePatient) {
        // 새 환자 행
        if (cur) patients.push(cur);
        lastInst = rawInst || ""; // 새 환자 시작 시 진료기관 초기화
        ...
        // 동일 환자의 추가 진료 행 (차트번호 공백·반복·병합)
        const hasDate = !!excelDateToISO(dateRaw);
        // 날짜도 없고 진료기관도 없는 행(빈 합계 행)은 건너뜀
        if (!hasDate && !rawInst) continue;
        const isoDate = hasDate ? excelDateToISO(dateRaw) : lastDate;
    ...
    results.push({ reportMonth, patients: validPatients, grandTotal, sheetName: shName });
  }
...
export async function parseTaxExcel(file) {
  ...
  for (const shName of wb.SheetNames) {
    ...
    results.push({ reportMonth, groups, grandTotal, sheetName: shName });
  }
