...
export function WardDataProvider({ children }) {
  ...
  useEffect(() => {
    ...
    const u6 = onValue(ref(db, "logs"), snap => {
      ...
      // 기존 데이터: 배열 형태 (set 통째 덮어쓰기 시절). 신규: push 키 객체.
      // RTDB 룰 강화(2026-04-26) 후 새 entry 는 push 키. 둘 다 호환되게 처리.
      const arr = Array.isArray(val) ? val : Object.values(val);
      // ts(ISO) 내림차순 — 신규가 위. 마이그레이션 전 배열 데이터도 같은 정렬 적용.
  ...
  // ── 공유 함수: addLog ──────────────────────────────────────────────────
  // RTDB 룰 강화(2026-04-26): logs 가 append-only 로 잠겨 기존 entry 변경·삭제 불가.
  // 따라서 set(전체배열) 패턴(=기존 데이터 덮어쓰기) 은 거부됨. push 키 신규 추가만 허용.
  // 낙관적 갱신은 그대로 유지(UI 즉시 반응) + onValue 로 정합성 보정.
  const addLog = useCallback(async (entry) => {
    ...
    try {
      ...
      console.error("addLog write failed:", e);
    }
  }, []);
  ...
  // intent: 'move' (newSlotKey 있음) | 'cancel' (예약 취소) | 'discharge' (퇴원)
  //   cancel  → status="상담중"      (환자가 아직 상담 단계로 돌아감)
  //   discharge → status="입원완료"  (입원 후 퇴원 → 과거 이력화, auto-restore 차단)
  const syncConsultationOnSlotChange = useCallback(async (fromSlotKey, personName, consultationId, newSlotKey, updatedDates, intent) => {
    const matches = Object.entries(consultations).filter(([id, c]) => {
    ...
    if (newSlotKey) {
      // 이동: 매칭된 모든 consultation 의 reservedSlot 을 target 으로 갱신.
      //   과거: matches[0] 1건만 갱신 → 같은 사람의 중복 상담(import_ 계열) 이 남아있으면
      //   auto-restore 가 원래 slot 으로 되돌려 "드래그 후 원위치 잔존" 버그 발생.
      //   날짜(admitDate/dischargeDate) 는 호출자가 넘긴 경우만 모두에게 동일하게 적용.
      for (const [cId, c] of matches) {
      ...
      // 퇴원/취소: 매칭되는 모든 consultation 상태 갱신 (import_ 계열 중복 포함)
      const newStatus = intent === 'discharge' ? '입원완료' : '상담중';
      for (const [cId, c] of matches) {
        if (c.status === '취소' || c.status === '입원완료') continue;
        await set(ref(db, `consultations/${cId}`), { ...c, reservedSlot: null, status: newStatus });
      }
    }
  }, [consultations]);
