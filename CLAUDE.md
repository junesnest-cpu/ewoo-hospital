...
## 🚨 오류/버그 수정 작업 규칙 (Claude 반드시 준수)
사용자가 "오류", "버그", "이상하다", "안 된다" 등 문제 해결을 요청하면 **먼저 [docs/HOTFIX.md](./docs/HOTFIX.md) 를 읽어** 동일/유사 증상이 기록돼 있는지 확인한다. 있으면 그 원인·가드·복구 도구를 우선 활용한다. 해결 후에는 HOTFIX.md 맨 위에 새 섹션(`## YYYY-MM-DD — 제목`)을 추가하여 증상·원인·수정 커밋·재발 방지 가드·검증/복구 도구·연관 관찰을 기록한다. 기록을 누락하면 다음 번에 같은 버그를 처음부터 디버깅하게 된다.
